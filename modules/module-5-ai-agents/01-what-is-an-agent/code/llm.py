...
def _groq(messages, model, temperature) -> str:
    ...
    try:
        ...
        # Tool-native models (for example gpt-oss) may emit a NATIVE tool call even
        # though this plain path sent no tools (the manual-JSON protocol of class 5.1).
        # Groq then returns 400 tool_use_failed with the model's intended call in
        # `failed_generation`. Recover it as our {"tool": ..., "arguments": ...} text
        # so the hand-rolled loop can still parse it, instead of crashing.
        recovered = _recover_groq_tool_call(e)
...
def _recover_groq_tool_call(err):
    """If `err` is Groq's tool_use_failed error, return the model's intended call as a
    manual-JSON string {"tool": name, "arguments": {...}}; else return None so the
    caller re-raises. This bridges tool-native models into the class 5.1 text protocol."""
    import json as _json
    import re as _re

    body = getattr(err, "body", None)
    data = body.get("error", body) if isinstance(body, dict) else None
    if data is None:                              # fall back to parsing the message text
        m = _re.search(r"\{.*\}", str(err), _re.S)
        if m:
            try:
                data = _json.loads(m.group(0)).get("error")
            except Exception:
                data = None
    if not isinstance(data, dict) or data.get("code") != "tool_use_failed":
        return None
    ...
    if isinstance(args, str):
        try:
            ...
            pass
    return _json.dumps({"tool": call.get("name") or call.get("tool"), "arguments": args})
